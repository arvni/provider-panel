...
const Index = () => {
    const {
        permissions: { data: permissionsData, ...pagination },
        request,
    } = usePage().props;
        ...
        },
        {
            ...
            render: (row) => (
                <Stack direction="row" spacing={1}>
                    <IconButton onClick={editPermission(row.id)}>
                        <Edit />
                    </IconButton>
                    <IconButton onClick={deletePermission(row)}>
                        <Delete />
                    </IconButton>
                </Stack>
            ),
    ...
    return (
        <>
            ...
            />
            <AddForm
                title={`${!edit ? "Add New" : "Edit"} Permission`}
                loading={processing}
                open={openAddForm}
                values={data}
                reset={reset}
