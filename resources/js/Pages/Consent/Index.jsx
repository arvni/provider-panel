...
const Index = () => {
    const {
        consents: { data: consentsData, ...pagination },
        request,
    } = usePage().props;
    ...
    const columns = [
        ...
        },
        {
            ...
            render: (row) =>
                row.file && (
                    <IconButton
                        href={route("file", { id: row.id, type: "consent" })}
                        target="_blank"
                    >
                        <RemoveRedEye />
                    </IconButton>
        ...
        },
        {
            ...
            render: (row) => (
                <Stack direction="row" spacing={1}>
                    <IconButton onClick={editConsent(row.id)}>
                        <Edit />
                    </IconButton>
                    <IconButton onClick={deleteConsent(row)}>
                        <Delete />
                    </IconButton>
                </Stack>
            ),
    ...
    const handleSubmitForm = () =>
        post(edit ? route("admin.consents.update", data.id) : route("admin.consents.store"), {
            onSuccess: () => {
