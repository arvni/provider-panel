...
const Index = () => {
    const {
        sampleTypes: { data: sampleTypesData, ...pagination },
        request,
    } = usePage().props;
        ...
        },
        {
            ...
            render: (row) => (
                <Stack direction="row" spacing={1}>
                    <IconButton onClick={editSampleType(row.id)}>
                        <Edit />
                    </IconButton>
                    <IconButton onClick={deleteSampleType(row)}>
                        <Delete />
                    </IconButton>
                </Stack>
            ),
    ...
    return (
        <>
            ...
            />
            <DeleteForm
                title={`${sampleType?.name} SampleType`}
                agreeCB={handleDestroy}
                disAgreeCB={handleCloseDeleteForm}
                openDelete={openDeleteForm}
            />
            <AddForm
                title={`${!edit ? "Add New" : "Edit"} Sample Type`}
                loading={processing}
                open={openAddForm}
                values={data}
                reset={reset}
