 ...
 * @returns {JSX.Element}
 */
const Filter = ({ columns, onChange, filter }) => {
    ...
    const hasActiveFilter = useCallback(
        (col) => {
            ...
            const fieldName = col.filter.name.includes(".")
                ? col.filter.name.split(".")[0]
                : col.filter.name;
    ...
    const renderSingleFilterField = useCallback(
        (field, columnIndex) => (
            <RenderFormField
    ...
    const renderFilterCell = useCallback(
        (col, index) => {
            ...
            if (isActive) {
                return (
                    <Badge color="primary" variant="dot" overlap="circular" sx={styles.badgeStyles}>
